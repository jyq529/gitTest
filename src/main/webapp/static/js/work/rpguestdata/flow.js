...
function setCreateDate(){
	...
	$("#flow_queryTime").val(""+date.getFullYear() + "-" + addZero(date.getMonth()+1));
}
/*重置*/
function flowResetform(){
	setCreateDate();
	//query_today = "";//姓名
}
/*条件查询*/
function flowQueryConditions(){
	refreshRpGuestFlowData();
...
/*加载列表数据*/
function refreshRpGuestFlowData(page,query_flag){
	/*if(query_flag){
		query_today = $("#flow_queryTime").val();//查询时间
	}*/
	if(($("#flow_queryTime").val()=="")||($("#flow_queryTime").val()==null)){
		showMsg({
			msg : "月份不能为空！",
			type : "error"
		});
	...
	var requestParams = { //发送参数
			...
			rows : rows,//每页显示的记录数
			queryFlag :'flow',
			queryTime: $("#flow_queryTime").val(),//当前时间
	...
	$.ajax({
		...
		success : function(result) {
			...
			$.each(arrX,function(index,value){
				$("<col style=\"width:50px;\"/>").appendTo($("#datagrid"));
			});
			var thB = $("<thead></thead>");
			...
			var toB = $("<tbody id='flow_data_tbody'></tbody>");//tbody
			toB.appendTo($("#datagrid"));
			var trB = $("<tr></tr>");//tbody中增加行-显示每列的数据
			trB.appendTo($("#flow_data_tbody"));
			...
			obj.data = arrY;//Y轴数据
			obj.xAxis = arrX;//X轴数据
			obj.chartName = this.setChartName();//统计图标题名称
			setCharts(obj);
			
			var trB2 = $("<tr></tr>");//tbody中增加行-显示数据单位说明
			trB2.appendTo($("#flow_data_tbody"));
			if (userModel.roleId == "1"){//管理员-流量以GB计算
				$("<td><div style=\"width:50px;\">单位:GB</div></td>").appendTo(trB2);
			}else{//管理员以外的角色-流量以MB计算
				$("<td><div style=\"width:50px;\">单位:MB</div></td>").appendTo(trB2);
			}

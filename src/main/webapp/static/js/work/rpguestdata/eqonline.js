...
function setCreateDate(){
	...
	$("#eqonline_queryTime").val(""+date.getFullYear() + "-" + addZero(date.getMonth()+1));
}
/*重置*/
function eqOnlineResetform(){
	setCreateDate();
	//query_today = "";//姓名
}
/*条件查询*/
function eqOnlineQueryConditions(){
	refreshEqOnlineRpGuestData();
...
/*加载列表数据*/
function refreshEqOnlineRpGuestData(page,query_flag){
	/*if(query_flag){
		query_today = $("#flow_queryTime").val();//查询时间
	}*/
	if(($("#eqonline_queryTime").val()=="")||($("#eqonline_queryTime").val()==null)){
		showMsg({
			msg : "月份不能为空！",
			type : "error"
		});
	...
	var rows = 10; //暂定每页显示10行
	var requestParams = { //发送参数
			page : page ==null?1:page,//当前页
			rows : rows,//每页显示的记录数
			queryFlag :'eqOnline',
			queryTime: $("#eqonline_queryTime").val(),//当前时间
			
	};
	$.ajax({
		type : 'POST',
		url : ctx+"/work/trpguestdata/queryRpGuestEqonlineData",
		data : requestParams,
		async : true,
		success : function(result) {
			...
			var toB = $("<tbody id='eqOnline_data_tbody'></tbody>");//tbody
			toB.appendTo($("#datagrid"));
			var trB = $("<tr></tr>");//tbody中增加行
			trB.appendTo($("#eqOnline_data_tbody"));
			...
			trB2.appendTo($("#eqOnline_data_tbody"));
			$("<td><div style=\"width:50px;\">单位:台</div></td>").appendTo(trB2);
		},
		error : function() {
		}
	});
